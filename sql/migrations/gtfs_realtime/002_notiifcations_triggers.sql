...
-- =========================================
CREATE OR REPLACE FUNCTION gtfs_rt.notify_trip_update() RETURNS TRIGGER AS $$
DECLARE
...
BEGIN
    ...
    -- Send trip-specific notification
    IF NEW.trip_id IS NOT NULL THEN
        PERFORM pg_notify(
            format('trip_updates:%s:%s', v_feed_message.source_id, NEW.trip_id),
            json_build_object(
                'type', 'trip_update',
                'trip_id', NEW.trip_id,
...
DROP TRIGGER IF EXISTS notify_trip_update ON gtfs_rt.trip_updates;
CREATE TRIGGER notify_trip_update
    AFTER INSERT OR UPDATE ON gtfs_rt.trip_updates
    FOR EACH ROW
